...
$(function(){
	$("#selCount").change(function(){
		   ...
		   alert(size);
		    location.href="${pageContext.request.contextPath}/orderShipping/checkShippingBySize?rows="+size+"&page="+${page.page};  
		})
	$("#del").click(function(){
		var str="";
		...
		alert(str);
	 	 $.post("${pageContext.request.contextPath}/orderShipping/delShipping",{"str":str,"page":"${page.page}","size":"${page.size}"},function(page){
...
<div id="head">
    <div id="nav">
        <span>
            <a href="">首页</a> >> <a href="">发货管理</a> （共${page.total }条记录）
        </span>
    </div>
...
<div id="bodyer">
    ...
    <table>
      ...
      <tr>
            ...
            <td>${l.delivery_corp }</td>
             <td>
             ${l.tracking_no }
              <td>${l.consignee }</td>
                <td><fmt:formatDate value="${l.create_date }" pattern="yyyy-MM-dd"/></td>
            <td><a href="${pageContext.request.contextPath }/orderShipping/ShippingDetails?id=${l.id}">[查看]</a></td>
        </tr>
      </c:forEach>
    </table>
   <itcast:page url="${pageContext.request.contextPath }/orderShipping/checkShippingBySize"/>
</div>
</body>
</html>