...
<meta http-equiv="content-type" content="text/html; charset=utf-8" />
		...
		<meta name="copyright" content="SHOP++" />
			<meta name="keywords" content="${xxpc.name }" />
			<meta name="description" content="${xxpc.name }" />
...
<script type="text/javascript">
	$(function(){
		...
		var tagIds="${param.tagIds}";
	 	...
	 	//热销  最新checkbox
	 	$("#tag1").click(function(){
	 		window.location.href="${pageContext.request.contextPath }/sortShop/sort?&type=${type}&temp=${temp}&tagIds=month_sales";
	 	})
	 	$("#tag2").click(function(){
	 		window.location.href="${pageContext.request.contextPath }/sortShop/sort?&type=${type}&temp=${temp}&tagIds=modify_date";
	 	})
	 	//价格区间提交按钮
	 	$("#btn").click(function(){
	 		var startprice=$("#startPrice").val();
	 		var endprice=$("#endPrice").val();
	 		 window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=price-desc&type=${type}&temp=${temp}&startPrice="+$("#startPrice").val()+"&endPrice="+$("#endPrice").val(); 
	 	})
	 	
		$("#price").click(function(){
			// asc desc
			if($(this).prop("class")=="asc"){
				 window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=price-desc&type=${type}&temp=${temp}"; 
			}
			else{
				 window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=price-asc&type=${type}&temp=${temp}"; 
			}
			})
		$("#sales").click(function(){
			// asc desc
			if($(this).prop("class")=="asc"){
				window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=sales-desc&type=${type}&temp=${temp}";
			}
			else{
				window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=sales-asc&type=${type}&temp=${temp}";
			}
		})
		$("#score").click(function(){
			// asc desc
			if($(this).prop("class")=="asc"){
				window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=score-desc&type=${type}&temp=${temp}";
			}
			else{
				window.location.href="${pageContext.request.contextPath }/sortShop/sort?str=score-asc&type=${type}&temp=${temp}";
			}
		})
...
<div class="container productList">
		...
		<div class="span18 last">
			...
			<div id="div1" class="path">
					...
					<div id="sort" class="sort">
						<div id="orderSelect" class="orderSelect">
							...
							<div class="popupMenu">
								<ul>
										<li>
											<a href="${pageContext.request.contextPath }/sortShop/sort?str=is_top-desc&type=${type}&temp=${temp}" id="topDesc">置顶降序</a>
										</li>
										<li>
											<a href="${pageContext.request.contextPath }/sortShop/sort?str=price-asc&type=${type}&temp=${temp}" id="priceAsc">价格升序</a>
										</li>
										<li>
											<a href="${pageContext.request.contextPath }/sortShop/sort?str=price-desc&type=${type}&temp=${temp}" id="priceDesc">价格降序</a>
										</li>
										<li>
											<a href="${pageContext.request.contextPath }/sortShop/sort?str=sales-desc&type=${type}&temp=${temp}" id="salesDesc">销量降序</a>
										</li>
										<li>
											<a href="${pageContext.request.contextPath }/sortShop/sort?str=score-desc&type=${type}&temp=${temp}" id="scoreDesc">评分降序</a>
										</li>
										<li>
											<a href="${pageContext.request.contextPath }/sortShop/sort?str=modify_date-desc&type=${type}&temp=${temp}" id="dateDesc">日期降序</a>
										</li>
								</ul>
							</div>
						</div>
				...
				<div id="div4" class="result table clearfix" >
						<ul id="shopList">
							<c:forEach items="${page.rows}" var="l">
									<li>
										<a href="${pageContext.request.contextPath }/shopContent/shop?id=${l.id }">
				...
				<!-- 主要标签查询  -->
				<c:if test="${controller=='main' }">
				<itcast:page url="${pageContext.request.contextPath }/hc/main" />
				</c:if>
				<!-- 条件查询 -->
				<c:if test="${controller=='sort'}">
				<itcast:page url="${pageContext.request.contextPath }/sortShop/sort" />
				</c:if>
				<c:if test="${controller=='search'}">
				<itcast:page url="${pageContext.request.contextPath }/sortShop/search" />
				</c:if>
				<!-- 热门 以及最新 -->
				<c:if test="${controller=='shopSort'}">
				<itcast:page url="${pageContext.request.contextPath }/sortShop/shopSort" />
				</c:if>
				<!-- 搜索 -->
				<c:if test="${controller=='searchAll'}">
				<itcast:page url="${pageContext.request.contextPath }/search/searchAll" />
				</c:if>
				 </c:if> 
				...
				<div id="div6" >
					<ul>
						<c:forEach items="${listBrand }" var="lb">
								<li >
									<a href="${pageContext.request.contextPath}/sortShop/brand?id=${lb.id}">
