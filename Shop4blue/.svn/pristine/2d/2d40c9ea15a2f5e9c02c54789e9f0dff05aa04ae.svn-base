...
$(function(){
	$("#selCount").change(function(){
		   ...
		   location.href="${pageContext.request.contextPath}/orderReturn/checkRefundsBysize?rows="+size+"&page="+${page.page}; 
		})
	$("#del").click(function(){
		var str="";
		...
		if(len==0){
		 ...
		 $.post("${pageContext.request.contextPath}/orderReturn/delRefunds",{"str":str,"page":"${page.page}","size":"${page.size}"},function(page){
...
<div id="head">
    <div id="nav">
        <span>
            <a href="">首页</a> >> <a href="">退款管理</a> （共${page.total }条记录）
        </span>
    </div>
    ...
    <table>
      ...
      <tr>
            ...
            <td>${l.payment_method }</td>
             <td>
             <fmt:formatNumber type="number" value="${l.amount }" pattern="#.00"/></td>
              <td>${l.payee }</td>
                <td><fmt:formatDate value="${l.create_date }" pattern="yyyy-MM-dd"/></td>
            <td><a href="${pageContext.request.contextPath }/orderReturn/ReturnMoneyDetails?id=${l.id}">[查看]</a></td>
        </tr>
      </c:forEach>
    </table>
   <itcast:page url="${pageContext.request.contextPath }/orderReturn/checkRefundsBysize"/>
</div>
</body>
</html>